...
describe('AppController', () => {
  ...
  it('should process a message with valid user data and a correct answer when the user context is "Ask a new question"', async () => {
    ...
    expect(userService.findUserByMobileNumber).toHaveBeenCalledWith('1234567890',process.env.BOT_ID);
    expect(messageService.sendMessageForCorrectAns).toHaveBeenCalledWith('1234567890','english');
    expect(messageService.askQuestionButton).toHaveBeenCalledWith('1234567890','english',);
    expect(userService.updateUserQuestionLimit).toHaveBeenCalledWith('1234567890',process.env.BOT_ID,0,new Date().toISOString().split('T')[0],);
  });
  ...
  it('should process a message for asking name with valid user data and a valid context when the button and persistent menu response is null and body.text is set to "hi"', async () => {
    ...
    // Assertions
    expect(userService.findUserByMobileNumber).toHaveBeenCalledWith('1234567890',process.env.BOT_ID,);
    expect(messageService.askUserName).toHaveBeenCalledWith('1234567890','english');
    expect(userService.updateUserContext).toHaveBeenCalledWith('1234567890',process.env.BOT_ID,english.context[3]);
  });
  ...
  it('should process a message with valid user data and a valid context and button response', async () => {
        ...
        // Assertions
        expect(userService.findUserByMobileNumber).toHaveBeenCalledWith('1234567890', process.env.BOT_ID);
        expect(userService.updateButtonResponse).toHaveBeenCalledWith('1234567890', process.env.BOT_ID, english.context[0]);
        expect(userService.setUserPreferredLanguage).toHaveBeenCalledWith('1234567890', 'hindi', process.env.BOT_ID);
        expect(messageService.askQuestionButton).toHaveBeenCalledWith('1234567890', 'hindi');
      });
});
